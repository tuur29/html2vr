...
function setup() {
  ...
  scene = createNode('<a-scene class="html2vr-permanent" background="color: #cecece" />');
...
function getList() {
  return window.opener.document.querySelectorAll('a img');
}
...
function getDetail() {
  return window.opener.document.querySelector('h1');
}
...
function draw() {
  // first decide what to draw
  let type = 'list';
  if (window.opener.document.title.toLowerCase().includes('detail')) {
    type = 'detail';
  }
